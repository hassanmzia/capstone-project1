...
class CNEAv5(QObject):
    def __init__(self, bitstreamFile, data_queue):
        ...
        self.data_pipe = bytearray(65536*16*4)
        self.view = memoryview(self.data_pipe)
    ...
    def pipe_out_block(self, bank, data_length):
        ...
        self.xem.ReadFromBlockPipeOut(bank, 1024, data_pipe) # block_size = 256*4
    ...
    # ================= DAC =========================
    def pcb_config_write(self, reset, ref_data, temp_data, lpf_data, mux_data):
        if reset:
        ...
        self.send_wire(0x0C, mux_data, 0x0000_01FF)  # Mux Data, 9 bits
    ...
    def ads8688_init(self):
        self.ads8688_write_comd_reg(0x8500, 0x00) # Reset Program Registers
    ...
    def data_stream_init(self):
        ...
        if self.recording_enable:
            ...
            self.data_raw = self.fhandle_data_raw.create_earray(self.fhandle_data_raw.root, "data", tb.UInt16Atom(), shape=(0, 1))
    ...
    def data_stream_close(self):
        try:
            ...
            # self.fhandle_pcb.flush()
            # self.fhandle_ic.flush()
            self.fhandle_data_raw.flush()
    ...
    def data_stream_ic_ddr3(self,data_length, index):
        start_time= time.perf_counter()
        datain = self.pipe_out_block(0xA2, int(data_length))  # pipeout from DDR3, data_length,length =65536*speed_factor
        # print('data length: ', len(datain))
        # print('data shape: ', np.shape(datain))
        
        # ####### multiprocessing.Queue testing ###########
        if self.data_queue.full(): 
            self.overflow_counter += 1
            print('the queue is full: ', self.overflow_counter)
            try:
                while not self.program_exit:
                    self.data_queue.get_nowait()
            except Empty:
                pass
        self.data_queue.put(datain)
        # print('data put into queue: ', index)
        print(time.perf_counter()-start_time)
        
        # start_time= time.perf_counter()
        
        # if self.recording_enable:
        #     self.data_raw.append(np.reshape(datain, (-1, 1)))
        # print(time.perf_counter()-start_time)
